...
from evaluation import dice_coef, dice_coef_loss
...
class DataGenerator(keras.utils.Sequence):
    ...
    def __load__(self, imgName):
        ...
        imagePath = os.path.join(self.path,  'images_g', imgName)
        maskName =  imgName[:-4]+'mask.png'
        maskPath = os.path.join(self.path, 'masks_g', maskName)
...
def getSegmentation():
    ...
    imagePath = os.path.join(BASEPATH, 'images_g')
    ...
    saveModel(model, 'unet_germ')
    saveHistory(history, 'unet_germ')

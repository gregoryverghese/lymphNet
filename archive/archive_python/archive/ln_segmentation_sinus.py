...
from evaluation import dice_coef_loss, dice_coef_loss
...
class DataGenerator(keras.utils.Sequence):
    ...
    def __load__(self, imgName):
        ...
        imagePath = os.path.join(self.path,  'images_s', imgName)
        maskName =  imgName[:-4]+'mask.png'
        maskPath = os.path.join(self.path, 'masks_s', maskName)
...
def getSegmentation():
    ...
    imagePath = os.path.join(BASEPATH, 'images_s')
    ...
    saveModel(model, 'unet_sinus')
    saveHistory(history, 'unet_sinus')

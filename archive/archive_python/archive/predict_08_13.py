...
class WSIPredictions(object):
    ...
    '''
    def predict(self, image, mask, label, outPath, step=2048, threshold=0.5):
            
        x,y,x= K.int_shape(image)
          
        if x>step and y>step:
            imagepatches = [[image[i:i+step,j:j+step,:] for j in range(0, y,step)] for i in range(0, x, step)]
            maskpatches = [[mask[i:i+step,j:j+step,:] for j in range(0, x, step)] for i in range(0, x, step)]
            
            probabilities = [[self.model.predict(img) for img in images[i]] for i in range(len(images))]

            finalimage = np.vstack([np.hstack(i) for i in images])
            finalmask = np.vstack([np.hstack(i) for i in masks])
            finalpred = np.vstack([np.hstack(i) for i in probabilities])
            probabilities = tf.cast((probabilities>theshold), tf.float32)

        else:
            finalimage=image
            finalmask=mask
            probabilities = tf.cast((probabilities>threshold), tf.float32)

        #interpolation of masks for resizing at creation time
        #means we have non 1 values - NEED TO CHANGE AND DO
        #THIS AT SOURCE 
        finalmask = finalmask.numpy().astype(np.uint8)
        mask[mask!=0]=1

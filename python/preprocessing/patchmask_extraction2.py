...
class WSITiling():
	def __init__(self, tileDim, resizeDim, magLevel, magFactor, step, masks, imageDir, maskDir, outPath, software, drawBorder):
	...
	def getPatches(self, scan, ndpi, boundaries, annotations, flag=False):
		...
		for w in range(boundaries[0][0], boundaries[0][1], self.step*self.magFactor):
			for h in range(boundaries[1][0], boundaries[1][1], self.step*self.magFactor):
				...
				paths = [(Path(v),k) for k, v in annotations.items()]
				if flag!='IFR':
						contains = list(map(lambda x: x[0].contains_point([w,h]), paths))	
						if True in contains and np.mean(patch) < 200:
							p = paths[contains.index(True)][0]
							k = paths[contains.index(True)][1]
					...
					filters = [self.filterPatches(p[0], w, h, 0.25) for p in paths]
	...
	def getTiles(self, ndpiPath, annotationPath):
		...
		#ndpiFiles = [f for f in ndpiFiles if 'U_100278' in f]                
		...
		for i, ndpi in enumerate(ndpiFiles):
			...
			annotation, boundaries = getattr(self, 'get'+self.software+'Annotations')(ndpi, annotationPath)
...
if __name__=='__main__':
	...
	tiling = WSITiling(tileDim, resizeDim, magLevel,  magFactor, step, masks, imageDir, maskDir, outPath, software, drawBorder)

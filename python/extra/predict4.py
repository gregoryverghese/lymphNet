...
def processTestData(image,mask): 
    ...
    images,mask = applyNormalization(image,mask)            
    
    return image,mask
 

def generatePredictions(model,image,mask,step):
    ...
    predict = WSIPredictions(model,image,mask,step)
    ...
    for data in dataset:
        ...
        names.append(label)
        image,mask=processTestData(image,mask)
        
        predict=WSIPredictions(model,image,mask,step)
        
        prediction=predict.(0.95)

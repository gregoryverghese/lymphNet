...
def patchPredict(images, model, feature, modelName, magLevel, magFactor, patchPath, maskPath, outPath):
    ...
    for imgName in images:
        ...
        for i, img in enumerate(patchFiles):

            m = os.path.join(maskPath, str(magLevel), feature, imgName, os.path.basename(img)[:-4]+'_masks.png')
            ...
            fig.savefig(os.path.join(figPath, os.path.basename(img[:-4]) + '_pred.png'))
            cv2.imwrite(os.path.join(outPatchPath, os.path.basename(img[:-4]) + '_pred.png'), prediction)
            plt.close()

...
from custom_loss_functions import weightedCategoricalCrossEntropy, weightedBinaryCrossEntropy, focalLoss, diceLoss, diceBCELoss
...
def trainSegmentationModel(args):
    ...
    trainNum = tfrecord_read_2.getRecordNumber(trainFiles)
    validNum = tfrecord_read_2.getRecordNumber(validFiles)
    testNum = tfrecord_read_2.getRecordNumber(testFiles)
    ...
    history = model.fit(tfrecord_read_2.getShards(trainFiles, batchSize=batchSize, imgDims=imgDims, dataSize=trainNum,  augmentations=augment), steps_per_epoch=trainSteps, epochs=epoch,
                      validation_data=tfrecord_read_2.getShards(validFiles, batchSize=batchSize, imgDims=imgDims, dataSize=validNum), validation_steps=validSteps)
    
    #model.save(modelName+'.h5')
    saveModel(model, modelName)
    saveHistory(history, modelName+'_hist')
    
    testdataset = tfrecord_read_2.getShards(testFiles, imgDims, batchSize=batchSize, dataSize=testNum, test=True)

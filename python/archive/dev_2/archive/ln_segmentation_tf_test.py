...
def trainSegmentationModel(args):
    ...
    model = train(model, optimizer, loss, trainDataset, trainSteps, epoch)
    model.compile(optimizer=optimizer, loss=loss, metrics=[diceCoef])

    #history = model.fit(tfrecord_read.getShards(trainFiles, imgDims=imgDims, batchSize=batchSize, dataSize=trainNum,  augmentations=augment), steps_per_epoch=trainSteps, epochs=epoch,
                      #validation_data=tfrecord_read.getShards(validFiles, imgDims=imgDims, batchSize=batchSize, dataSize=validNum), validation_steps=validSteps)

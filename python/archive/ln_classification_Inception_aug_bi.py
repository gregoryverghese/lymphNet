...
def getClassification():
    ...
    for i in range(len(models)):
        saveModel(models[i], MODELNAMES[i]+'_2')
        saveHistory(histories[i], MODELNAMES[i]+'_2')

    predictions = list(map(lambda x: x.predict_generator(testGenerator, steps=testGenerator.samples), models))
    classPredictions = list(map(lambda x: (np.array(x) > 0.5).astype('uint8'), predictions))

    resultsDict = {'Inception': classPredictions[0], 'TRUE': testGenerator.classes}
    results = pd.DataFrame(resultsDict)
    results.to_csv('/home/verghese/results_Inception_aug_bi_2.csv')
    results.to_csv('/home/verghese/output/results_Inception_aug_bi_2.csv')

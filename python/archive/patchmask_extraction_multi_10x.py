...
labelFileKey={0: 'FOLLICLE', 1: 'GERMINAL CENTRE', 2: 'SINUS'}
...
patchPath = '/SAN/colcc/WSI_LymphNodes_BreastCancer/Greg/data/patches/segmentation/10x/images'
maskPath = '/SAN/colcc/WSI_LymphNodes_BreastCancer/Greg/data/patches/segmentation/10x/masks'
    ...
    for ndpi in ndpiFiles:
        ...
        for w in range(boundaries[0][0], boundaries[0][1], 500):
            for h in range(boundaries[1][0], boundaries[1][1], 500):
                patch = scan.read_region((w-2000, h-2000), 2, (1000, 1000))
                ...
                if (img2.shape == (4000, 4000, 3) and np.mean(patchRGB) < 200) and np.mean(patchRGB) > 50:
                        patchRGB.save(os.path.join(patchPath, os.path.basename(ndpi)[:-5])+'_'+str(w)+'_'+str(h)+'.png')

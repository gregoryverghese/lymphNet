...
EPOCHS = 15
MODELNAMES = ['VGG16']
BASEDIR = '/SAN/colcc/WSI_LymphNodes_BreastCancer/Greg/data/patches/classification/multiclass'
RATIO = 0.8
WEIGHTS = 'imagenet'
METRICS = ['acc']
...
def getClassWeights(trainGenerator):
    ...
    print('hellllooooooo',np.unique(trainGenerator))
...
def getClassification():
    ...
    if len(os.listdir(trainSinus)) == 1:
        ...
        numSplitFollcile = utilities.MoveFiles(BASEDIR, 'FOLLICLE', trainFollcile, validFollcile, testFollicle , RATIO)
        ...
        print('Sinus \n train: {} test {} valid {} \n Germinal \n train: {} test {} valid {} \n FOLLICLE \n train: {} test {} valid {}'.format(numSplitSinus[0],
                        ...
                        numSplitFollicle[0], numSplitFollicle[1], numSplitFollicle[2]))
    ...
    for i in range(len(models)):
        saveModel(models[i], MODELNAMES[i]+'_multiaug')
        saveHistory(histories[i], MODELNAMES[i]+'_multiaug')
    ...
    resultsDict = {'VGG16': classes[0], 'TRUE': labels}
    results = pd.DataFrame(resultsDict)
    results.to_csv('/home/verghese/results_VGG16_multi_aug.csv')
    results.to_csv('/home/verghese/output/ln_classification_bin_1_VGG16_multi_aug.csv')

...
def getPredictions(model, testFiles, maskPath, basePath, outPath):
    ...
    for file in testFiles:
        print(os.path.join(maskPath, os.path.basename(file)[:-4]))
        ...
        cv2.imwrite(os.path.join(outPath, os.path.basename(file)) + '_annot.png', combined)
    ...
    evaluationDf.to_csv('evaluation_bi.csv')
...
def getSegmentation(basePath, imagePath, maskPath, outPath, testSize=0.1):
    ...
    pickle.dump(model, open('randomforest_bi.sav', 'wb'))
...
if __name__=='__main__':
    ...
    outPath = '/home/verghese/output/segmentation/mlpixelwise_bi'
